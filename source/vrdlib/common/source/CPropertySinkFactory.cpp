...
{  
   auto const map([]
   {
      std::map<std::string, std::function<std::unique_ptr<API::IPropertySink>(boost::filesystem::path, std::unique_ptr<API::IConflictHandler>, API::IPropertySink::Mode)>> m;
   ...
   std::unique_ptr<API::IPropertySink> CPropertySinkFactory::create(boost::filesystem::path filePath)
   {
