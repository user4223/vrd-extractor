...
namespace VRD
{
   CContext::CContext(Utility::CStreamReader &reader)
       : Common::CPropertyHandler(), m_pReader(), m_reader(reader), m_logger(log4cxx::Logger::getLogger("VRD.CContext")), m_descriptors()
   {
   }

   CContext::CContext(std::unique_ptr<Utility::CStreamReader> reader)
       : Common::CPropertyHandler(), m_pReader(std::move(reader)), m_reader(*m_pReader), m_logger(log4cxx::Logger::getLogger("VRD.CContext")), m_descriptors()
   {
   }
   ...
   {
      std::for_each(m_descriptors.begin(), m_descriptors.end(), [=](auto const &entry)
                    { f(entry.second); });
   }
   ...
   std::ostream &operator<<(std::ostream &os, CContext const &c)
   {
      ...
      c.foreachDescriptor([&](auto const &d)
                          { lines.emplace_back(d.toString()); });
