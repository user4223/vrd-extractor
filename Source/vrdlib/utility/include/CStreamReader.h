...
{  
   ...
   {      
      CStreamReader(boost::filesystem::path const& path, ByteOrder byteOrder = ByteOrder::LittleEndian);
            
      template <typename T> T read();
      template <typename T> T readRaw(); ///< Without any possible turn for endianess
      std::vector<uint8_t> read(size_t bytes);
      
      template <typename T> T readAt(Absolute position);
      template <typename T> T readAt(Relative offset);
